...
mod reliability_tests {
    ...
    fn test_server_stability_over_time() {
        let mut server = match start_test_server() {
            Ok(server) => server,
            Err(_) => {
                println!("Warning: Could not start test server for stability test");
                return;
            }
        };
        ...
        println!("Stability test: {} successful, {} errors over 60 seconds", 
        ...
        assert!(request_count > 500, "Should handle reasonable number of requests: {}", request_count);
    }
